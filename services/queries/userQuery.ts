...
export const useSignUpUser = () => {
	...
	return useMutation({
		mutationFn: async (data: { name: string; email: string; password: string }) => {
			try {
				const res: AxiosResponse = await axiosClient.post(`/api/auth/register`, data);
...
export const useLoginUser = () => {
	...
	return useMutation({
		mutationFn: async (data: { email: string; password: string }) => {
			try {
				const res: AxiosResponse = await axiosClient.post(`/api/auth/login`, data);
		...
		mutationFn: async (data: { todaysVibe: string }) => {
			try {
				...
				if (axios.isAxiosError(error)) {
					...
					throw new Error("An unknown error while saving user's today's vibe!!");
				}
			}
		},
		onSuccess: () => {
...
export const useVerifyUser = () => {
	...
	return useMutation({
		mutationFn: async (data: { otp: string; email: string }) => {
			try {
				const res: AxiosResponse = await axiosClient.post(`/api/auth/verify`, data);
		...
		mutationFn: async (data: StudentPersonalInfoProps) => {
			try {
				...
				if (axios.isAxiosError(error)) {
					...
					throw new Error("An unknown error while saving user's initial data!!");
				}
			}
		},
		onSuccess: () => {
